...
const OPTIONS: Array<{ label: string; timeframe: CandleTimeframe }> = [{ label: "1m", timeframe: "1m" }, { label: "5m", timeframe: "5m" }, { label: "15m", timeframe: "15m" }, { label: "1h", timeframe: "1h" }, { label: "4h", timeframe: "4h" }, { label: "1D", timeframe: "1d" }];
...
export function DashboardCandlestickChart({ series: initialSeries }: { series: VerifiedCandleSeries }) {
  ...
  return <section className="dashboardMarketChart" aria-labelledby="dashboard-market-chart-title" data-status={series.status}>
    ...
    <footer><span>{timeframe === "4h" ? "Calculated locally from verified UTC-aligned 1-hour candles" : `${series.classification === "end_of_day" ? "End-of-day" : "Delayed"} verified provider series`}</span><span>Interactive intelligence only · no order execution</span></footer>
  </section>;
}
