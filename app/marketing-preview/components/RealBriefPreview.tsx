...
import { MorningMarketBrief } from "../../brief/components/MorningMarketBrief";
import { composeMorningMarketBrief } from "../../brief/lib/compose-market-brief.ts";
import { buildMarketBrief } from "../../lib/market-brief.ts";
...
import { formatUkTimestamp, type MarketQuote, type MarketSnapshot } from "../../lib/market-data.ts";
...
export function RealBriefPreview({ fixture }: { fixture: MarketingPreviewFixture }) {
  ...
  const support = fixture.levels.s1.toFixed(2);
  const resistance = fixture.levels.r1.toFixed(2);
  const expectedMove = `${fixture.levels.expectedMove.toFixed(2)} pts (illustrative range)`;
  ...
  const sessionLevels = deriveSessionReferenceLevels(candleSeries.candles, Math.floor(now / 1000));
  const model = composeMorningMarketBrief({
    ...
    dataAgeLabel: "EXAMPLE ONLY · 10-minute illustrative delay",
  ...
  return (
    <MemberShell active="brief" className="marketingRealMemberPreview">
      <aside className="dashPartialBanner" role="status">
        <strong>EXAMPLE-ONLY MEMBER EXPERIENCE</strong>
        <span>All figures, levels and commentary on this private preview are illustrative — not live market data.</span>
      </aside>
      <MorningMarketBrief
        model={model}
        insight={insight}
