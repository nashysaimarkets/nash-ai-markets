...
export function OnboardingForm({ initialPreferences = null, updating = false }: OnboardingFormProps) {
  ...
  return <form className="onboardingForm" onSubmit={submit}>
    ...
    <fieldset><legend>2. Market interests</legend>{[["futures","Index futures"],["equities","Equities and ETFs"],["macro","Macro and rates"],["volatility","Volatility"]].map(([value,label]) => <label key={value}><input type="checkbox" checked={interests.includes(value)} onChange={() => toggle(value)} />{label}</label>)}</fieldset>
