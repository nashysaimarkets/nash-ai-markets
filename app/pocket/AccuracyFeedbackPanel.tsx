...
type AccuracyFeedbackPanelProps = { analysis: AccuracyAnalysis; sample?: boolean; onApplyCorrection: (feedback: AccuracyFeedback) => void; onReanalyse: () => void; reanalysing?: boolean };
...
function getAccuracyFeedbackSnapshot() {
  ...
  let raw: string | null;
  try { raw = window.localStorage.getItem(ACCURACY_STORAGE_KEY); } catch { return cachedAccuracyFeedback; }
...
export default function AccuracyFeedbackPanel(props: AccuracyFeedbackPanelProps) {
  // The parent keys this panel by chart session. Corrections change values, not its identity.
  return <AccuracyFeedbackForAnalysis {...props} />;
}

function AccuracyFeedbackForAnalysis({ analysis, sample = false, onApplyCorrection, onReanalyse, reanalysing = false }: AccuracyFeedbackPanelProps) {
  ...
  const save = (verdict: AccuracyFeedback["verdict"]) => {
    ...
    try {
      ...
      cachedAccuracyFeedbackRaw = raw;
      setStorageMessage("");
      // A derived benchmark index must never prevent the feedback itself from saving.
      try { localStorage.setItem("pocket-bullseye-benchmark-candidates-v1", JSON.stringify(benchmarkCandidates(next))); } catch {}
    } catch {
      setStorageMessage("Device storage is unavailable. This feedback and correction are kept for this session only.");
    }
    cachedAccuracyFeedback = next; notifyAccuracyFeedback();
  ...
  return <section id="bullseye-feedback" className="psAccuracy" data-mode={mode}>
    <header><div><span>Result accuracy</span><strong>Help refine this read</strong></div><b data-review={mode === "SAVED" ? "complete" : "pending"}>{mode === "SAVED" ? (savedEntry?.verdict === "ACCURATE" ? "CONFIRMED" : "CORRECTED") : "AWAITING REVIEW"}<small>{summary.total ? `HISTORY · ${summary.rate}% FROM ${summary.total} RATED` : "RATE THIS RESULT BELOW"}</small></b></header>
    {mode === "IDLE" ? <><p>{sample ? "Rate your own chart after a scan. This example uses fictional data." : "Did Pocket read this chart correctly?"}</p><div className="psAccuracyChoice"><button type="button" data-tone="good" disabled={sample} onClick={() => save("ACCURATE")}>✓ ACCURATE</button><button type="button" data-tone="fix" disabled={sample} onClick={() => setMode("CORRECTING")}>! NEEDS CORRECTION</button></div></> : null}
    {mode === "CORRECTING" ? <div className="psAccuracyForm">
      <p>What single fact did Pocket get wrong?</p>
      <div className="psAccuracyCategories">{categories.map((category) => <button key={category.value} type="button" data-active={selected.includes(category.value)} aria-pressed={selected.includes(category.value)} onClick={() => toggle(category.value)}>{category.label}</button>)}</div>
