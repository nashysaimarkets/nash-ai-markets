...
export default async function Terminal() {
  ...
  return <main className="foxtrotTerminal" id="overview">
    ...
    <section className="ftWorkspace">
      ...
      <section className={`progressiveAccessBar progressiveAccessBar-${access.effectiveTier}`} aria-label="Membership access"><div>{access.effectiveTier === "elite" ? <span className="eliteAccessCrest" aria-hidden="true"><i /></span> : null}<TerminalBadge label={access.tier} tone={access.tier === "elite" ? "warning" : access.tier === "pro" ? "info" : "neutral"} /><strong>{access.effectiveTier.toUpperCase()} FEATURES ACTIVE</strong>{previewOffer?.active ? <span>{previewOffer.targetTier.toUpperCase()} preview active until reset</span> : null}</div><span>{access.effectiveTier === "elite" ? "Full Bullseye intelligence · decision · planning · diagnostics" : "Progressive access · deeper workflow when it earns its place"}</span></section>
