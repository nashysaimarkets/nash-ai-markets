...
if [[ -z "${NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY:-}" && -z "${NEXT_PUBLIC_SUPABASE_ANON_KEY:-}" ]]; then
  missing+=("NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY_or_NEXT_PUBLIC_SUPABASE_ANON_KEY")
fi
...
if [[ -n "${STRIPE_FOUNDING_PRO_PRICE_ID:-}" && "${STRIPE_FOUNDING_PRO_PRICE_ID}" == "${STRIPE_PRO_PRICE_ID:-}" ]]; then
  invalid+=("STRIPE_FOUNDING_PRO_PRICE_ID: must be a separate £12 recurring Price")
fi
