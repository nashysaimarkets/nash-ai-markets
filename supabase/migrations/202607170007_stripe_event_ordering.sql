alter table public.memberships
  add column if not exists last_stripe_event_created_at bigint
    check (last_stripe_event_created_at is null or last_stripe_event_created_at > 0);
...
revoke all on function public.sync_membership_from_stripe(text, text, text, text, text, timestamptz, text, integer, bigint) from public;
grant execute on function public.sync_membership_from_stripe(text, text, text, text, text, timestamptz, text, integer, bigint) to service_role;
