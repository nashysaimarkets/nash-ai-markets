...
test("freshness labels live, delayed and stale deterministically", () => {
  const candles = [candle(base, 100)];
  assert.equal(determineCandleFreshness(candles, base * 1000 + 9 * 60_000).status, "live");
...
test("provider failures remain fail closed and contract labels remain exact", async () => {
  ...
  assert.equal(unavailable.contract, "ESUSD");
...
test("provider response returns verified schema without exposing its credential", async () => {
  ...
  assert.equal(result.symbol, "ES");
  assert.equal(result.contract, "ESUSD");
  assert.equal(result.status, "live");
  assert.match(requested, /historical-chart\/5min/);
  assert.equal(JSON.stringify(result).includes("do-not-expose"), false);
});

test("15-minute and hourly candles are aggregated locally", () => {
  ...
  assert.equal(hourly[0]?.volume, 78);
});

test("EMA, VWAP, session range and reference levels are deterministic", () => {
  const candles = [candle(base, 100, 10), candle(base + 300, 102, 20), candle(base + 600, 104, 30)];
  ...
  assert.deepEqual(candleReferenceLevels(candles).map((level) => level.label), ["Session high", "Session low"]);
});
