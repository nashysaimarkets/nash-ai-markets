export const FED_H10_CSV_FIXTURE = `DATE,JRXWTFB_N.B,JRXWTFN_N.B,JRXWTFO_N.B
2026-07-23,120.9075,114.2626,129.3803
2026-07-24,120.7105,114.1340,129.1064
`;

export const FED_MONETARY_RSS_FIXTURE = `<?xml version="1.0"?>
<rss><channel>
<item>
<title>Federal Reserve issues FOMC statement</title>
<link>https://www.federalreserve.gov/newsevents/pressreleases/monetary20260729a.htm</link>
<pubDate>Wed, 29 Jul 2026 18:00:00 GMT</pubDate>
</item>
<item>
<title>Minutes of the Federal Open Market Committee, June 16-17, 2026</title>
<link>https://www.federalreserve.gov/newsevents/pressreleases/monetary20260708a.htm</link>
<pubDate>Wed, 08 Jul 2026 18:00:00 GMT</pubDate>
</item>
<item>
<title>Unrelated monetary policy announcement</title>
<link>https://www.federalreserve.gov/example.htm</link>
<pubDate>Thu, 30 Jul 2026 12:00:00 GMT</pubDate>
</item>
</channel></rss>`;
