...
test("analyse reserves the main deadline for the report and gives optional precision a shorter boundary", async () => {
  ...
  assert.match(source, /const POCKET_PRECISION_DEADLINE_MS = 82_000/);
...
test("report failure aborts and drains precision work before the route returns", async () => {
  ...
  assert.ok(rescueGate > precisionStart && rescueGate < contextStart);
  assert.ok(contextStart > rescueGate, "context precision must not start until the report succeeds");
